...
- **The release zip contains the DLL and nothing else.** `pack.ps1` (a workspace-synced generated
  file — don't edit it here) stages only `src/bin/Release/netstandard2.1/DeadReckoning.dll` into
  `BepInEx/plugins/DeadReckoning/`. Any loose asset the csproj's `DeployPlugin` target copies into the
  dev plugin folder exists **on this machine only** and never reaches a Nexus installer. Anything a
  user must have goes in as an `<EmbeddedResource>` with a pinned `<LogicalName>`.
- **This already shipped broken once.** v1.2.1 relied on a loose `track-icon.png` beside the DLL, so
  every Nexus install fell back to a text label on the Track buttons while the dev install looked
  fine. Fixed 2026-09-13 by embedding the PNG. The dev deploy no longer copies a loose icon,
  deliberately — the dev install must be byte-for-byte what ships, or the divergence hides the bug.
- **Verify the packaged path, not your install.** Before publishing, confirm there is no
