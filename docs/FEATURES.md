...
| Track a job | ✅ | "Seek Job" button on a job's Quest Log entry; steers to the job's hand-in NPC (`CompletionNpcConfigAsset`), auto-stops when the job completes or expires. Also fixed the stale green "Seeking Quest" button jobs used to inherit (jobs render via `ShowJobInfo`, which was unpatched). Added in v1.2.1. |
